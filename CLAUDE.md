...
- **A field's message carries a glyph.** With no hue, a success line and an error line are the same grey 14px text; `Field`/`SelectField`/`TextAreaField` take `success` alongside `error` and both render with their icon. A `hint` stays bare — it is not a state.
