...
export interface AvatarStackProps {
  ...
  size?: number
  className?: string
  /** Describes the whole stack; the individual avatars stay decorative. */
  label: string
}

/** Overlapping monograms — a compact way to stand for a set of holdings. */
export function AvatarStack({ items, max = 4, size = 28, className, label }: AvatarStackProps) {
  ...
  return (
    <span className={cn(styles.stack, className)} role="img" aria-label={label} style={{ '--stack-size': `${size}px` } as React.CSSProperties}>
      {shown.map((m, i) => (
        <span key={m + i} className={styles.item}>
          <Avatar label={m} monogram={m.slice(0, 1)} size={size} />
        </span>
      ))}
      {extra > 0 ? (
