...
référence — sans dépendance, la logique simulée servie en HTTP — qui répond à ce contrat :
...
plus la moindre donnée simulée, ouvre une session, lit les comptes, les titres et les poches
de devises, passe un ordre et vérifie que le flux d'évènements est bien **ouvert** et non
retombé sur l'interrogation périodique — le tout sur le réseau. C'est la seule preuve que
« brancher un back-end » fonctionne ; tout le reste teste l'application contre des chiffres
inventés.
